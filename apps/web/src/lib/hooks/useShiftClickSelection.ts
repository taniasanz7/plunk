...
interface ShiftClickHandlers<T> {
  /**
   * Attach to the selection control's `onClick` to capture the shift-key
   * state of the click. Pair with `onCheckedChange` below.
   */
  onClick: (event: React.MouseEvent) => void;
  /**
   * Attach to the selection control's `onCheckedChange`. On a plain click
   * this toggles the single row; when the preceding click had `shiftKey`
   * pressed AND a prior anchor row exists, every row between the anchor
   * and this row is set to the new checked state.
   */
 ...
 * shift-click row 5 (the new anchor is row 10, so this deselects 5–10).
 */
export function useShiftClickSelection<T>(table: Table<T>): ShiftClickHandlers<T> {
  ...
  const onCheckedChange = useCallback(
    (row: Row<T>, value: boolean | 'indeterminate') => {
      ...
      const anchorIndex = anchorRowIdRef.current
        ? rows.findIndex(r => r.id === anchorRowIdRef.current)
        : -1;
