 ...
 * Only string-keyed boolean entries are merged so a malformed stored value can't
 * leak unknown keys into the tanstack state. SSR-safe: reads happen inside an
 * effect, the initial state is always `defaults`.
 */
