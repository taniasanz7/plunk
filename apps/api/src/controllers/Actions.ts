    ...
    if (template) {
      ...
      if (!body) {
        ...
        if (layoutBody) {
          ...
          // Use a function replacer so any `$`-sequences in the template body
          // (e.g. `$&`, `$$`, `` $` ``, `$1`) are inserted literally rather
          // than being interpreted as String.replace special patterns.
