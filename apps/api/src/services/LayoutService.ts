...
export class LayoutService {
   ...
   *
   * Each row carries a `_count.templates` so the list view can show how many
   * templates reference the layout ("Used by"). Sorting is driven by the
   * shared `?sort=&dir=` contract; when no sort is supplied the default layout
   * floats to the top, then newest first.
   */
  public static async list(
    ...
    // Explicit sort wins; otherwise default layout first, then newest first.
    const orderBy: Prisma.LayoutOrderByWithRelationInput | Prisma.LayoutOrderByWithRelationInput[] = sort
      ? ({[sort.field]: sort.direction} as Prisma.LayoutOrderByWithRelationInput)
    ...
    return {
      data: layouts as LayoutWithUsage[],
      total,
      page,
      pageSize,
