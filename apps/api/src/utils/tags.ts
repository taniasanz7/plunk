...
export function normalizeTags(input: unknown): string[] | undefined {
  ...
  if (!Array.isArray(input)) {
    throw new HttpException(400, 'Tags must be an array of strings');
  }
  if (input.length > 50) {
    throw new HttpException(400, 'Tags must contain at most 50 entries');
  }
  ...
  for (const raw of input) {
    ...
    if (trimmed.length > 50) {
      throw new HttpException(400, 'Each tag must be at most 50 characters');
    }
    if (!trimmed) continue;
